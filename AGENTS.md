...
**What it is**: Pre-built knowledge graph of the project. Use instead of re-reading files for understanding.
...
1. Query the graph: `python -m graphify query "your question" --graph "graphify-out/graph.json"`
2. Only read specific files if needed

### Example Queries

```bash
# Find database config
python -m graphify query "PostgreSQL prisma database config" --graph "graphify-out/graph.json"

# Find auth related files
python -m graphify query "authentication better-auth session" --graph "graphify-out/graph.json"
```

---

## 6. Prisma Decimal Serialization
...
## 7. Git Workflow & Release Management
